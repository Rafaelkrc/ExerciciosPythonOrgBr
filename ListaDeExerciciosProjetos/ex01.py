...
with open('P:/ExerciciosPython/ExerciciosArquivos/usuarios.txt', 'r') as usuarios:
    for linha in usuarios:
        ...
        lista_de_dados.append((tamanho_em_disco, usuarios))


lista_de_dados.sort(reverse=True)
mostrar = int(input('Quantos usuários deseja mostrar: '))
lista_de_dados = lista_de_dados[:mostrar]
...
with open('P:/ExerciciosPython/ExerciciosArquivos/relatório.txt', 'w') as arquivo:
    ...
    for indice, dados in enumerate(lista_de_dados, start=1):
        tamanho_em_disco, usuarios = dados
        print(dados)
